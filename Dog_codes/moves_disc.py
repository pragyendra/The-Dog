	...
	while t < 50:
		for i in [1,3,5,7]:
			if i == 1:
				...
				continue
			dog.servo_dir[i].moveTimeWrite(((sin(t + phi))*(-dog.front[i])*move) + dog.stand2[i],100)
		t += dt
		time.sleep(0.1)
	stand2(dog)
	time.sleep(1.6)
...
def move_by(dog, id_m=None, dir_m=None, pos_m=None):
    if id_m:
        if dir_m == "front":
            sign = 1
        elif dir_m == "back":
...
def scratch_me(dog):
    if dog.current_pos == "scratch_me":
        ...
        pass
    move_by(dog, 3, "front", 120)
    time.sleep(1.6)
    t = 0
    dt = 0.1
    pos = dog.servo_dir[3].getPhysicalPos()
    while t<25:
        dog.servo_dir[3].moveTimeWrite(((sin(t))*dog.front[3]*15) + pos)
        time.sleep(0.05)
        t += dt
...
def jackson(dog):
    ...
    while step <10:
        if step % 2 == 0:
            ...
            for i in range(len(dog.servo_dir)):
                dog.servo_dir[i].moveTimeWrite(dog.straight[i],1500)
            time.sleep(1.7)
            step +=1
    stand2(dog)
    dog.current_pos = "stand2"
...
def side_side(dog):
    ...
    while step <30:
        if step % 2 == 0:
            for i in [0,1,6,7]:
                dog.servo_dir[i].moveTimeWrite(dog.stand2[i],500)
            for i in  [2,3,4,5]:
                dog.servo_dir[i].moveTimeWrite(dog.stand[i],500)
        else:
            for i in [0,1,6,7]:
                dog.servo_dir[i].moveTimeWrite(dog.stand[i],500)
            for i in  [2,3,4,5]:
...
def twerk(dog):
	...
	while step < 10:
		if up == 0:
			for i in [0,1,2,3]:
				dog.servo_dir[i].moveTimeWrite(dog.stand[i],100)
			for i in [4,5,6,7]:
				dog.servo_dir[i].moveTimeWrite(dog.stand2[i],100)
			up = 1
			time.sleep(0.6)
		else:
			for i in [0,1,2,3]:
